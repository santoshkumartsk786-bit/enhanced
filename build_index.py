...
def detect_sentiment(text):
    """
    Detect sentiment based on positive/negative keyword counts
    ...
    if pos_count == 0 and neg_count == 0:
        # No keywords found - neutral/unknown
        return 'neutral'

    # Return sentiment based on majority
    if pos_count > neg_count:
        return 'positive'
    elif neg_count > pos_count:
        return 'negative'
    else:
        # Tie - check sentiment intensifiers
        if any(word in text_lower for word in ['very', 'extremely', 'absolutely']):
            # Look at context around intensifiers
            if 'very good' in text_lower or 'extremely good' in text_lower:
                return 'positive'
            if 'very bad' in text_lower or 'extremely bad' in text_lower:
                return 'negative'

        # Default to positive on tie (optimistic approach)
        return 'positive'
...
if 'sentiment' in df.columns:
    ...
    if df['sentiment'].isnull().any():
        print(f"  WARNING: {df['sentiment'].isnull().sum()} unmapped sentiment values")
...
print(f"\nSentiment detection method: {'Existing column' if 'sentiment' in column_mapping.values() else 'Auto-detected from text'}")
print("\nNext steps:")
...
report = {
    ...
    'sentiment_source': 'existing_column' if 'sentiment' in column_mapping.values() else 'auto_detected',
